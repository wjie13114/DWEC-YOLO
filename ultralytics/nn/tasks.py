...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS + V10_DETECT_CLASS
                          + (OBB26, Pose26, Segment26, YOLOEDetect, YOLOESegment, YOLOESegment26))):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose, warehouse_manager=None)  # model, savelist
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS + V10_DETECT_CLASS
                         + (OBB26, Pose26, Segment26, YOLOEDetect, YOLOESegment, YOLOESegment26, WorldDetect))):  # includes all Detect subclasses
            s = 640  # 2x min stride
            ...
            def _forward(x):
                ...
                out = self.forward(x)
                if self.end2end:
                    out = out["output_2"] if self.fgi else out["one2many"]
                if isinstance(out, dict):  # training-mode output is {boxes, scores, feats, ...}
                    out = out.get("feats", out)
                elif isinstance(m, SEGMENT_CLASS + POSE_CLASS + OBB_CLASS) and isinstance(out, (list, tuple)):
                    out = out[0]
                return out
    ...
    def init_criterion(self):
        ...
        if getattr(self, "fgi", False):
            return E2EFGIDetectLoss(self)
        if getattr(self, "end2end", False):
            # Use E2ELoss for YOLOv26 models, E2EDetectLoss for legacy models
            m = self.model[-1]
            if isinstance(m, (OBB26, Pose26, Segment26, YOLOEDetect, YOLOESegment, YOLOESegment26)):
                return E2ELoss(self)
...
class SegmentationModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        if getattr(self, "end2end", False):
            return E2ELoss(self, v8SegmentationLoss)
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in ((
            ...
            C3x, RepC3, PSA, SCDown, C2fCIB, nn.Conv2d
            ...
            if m in (DySnakeConv,):
                c2 = c2 * 3
            if m in ((
                BottleneckCSP, C1, C2, C2f, C3k2, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fPSA, C2fCIB
            ) + C3K2_CLASS + A2C2F_CLASS + C2PSA_CLASS):
        ...
        elif m in ((WorldDetect, ImagePoolingAttn, OBB26, Pose26, Segment26, YOLOEDetect, YOLOESegment, YOLOESegment26, LRPCHead)
                   + DETECT_CLASS + V10_DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS):
            args.extend([reg_max, end2end, [ch[x] for x in f]])
            if m in SEGMENT_CLASS:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
        elif m is RTDETRDecoder:  # special case, channels arg must be passed in index 1
            args.insert(1, [ch[x] for x in f])
        elif m in {SpatialAttention, CBAM, CoordAtt, EUCB_SC}:
            c2 = ch[f]
            args = [c2, *args]
        else:
            c2 = ch[f]

        if isinstance(c2, list) and m not in set():
            is_backbone = True
            m_ = m
            m_.backbone = True
        else:
...
def guess_model_scale(model_path):
    ...
    try:
        if re.search(r'(hyper-yolo)([tnslmx])', Path(model_path).stem) is not None:
            ...
            return re.search(r"yolo(e-)?[v]?\d+([nslmx])", Path(model_path).stem).group(2)  # n, s, m, l, or x
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, Segment26, YOLOESegment, YOLOESegment26)):
                return "segment"
