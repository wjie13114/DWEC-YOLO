...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, mpdiou_hw=None):
        ...
        if self.use_wiseiou:
            #wiou = self.wiou_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask], ret_iou=False, ratio=0.7, d=0.0, u=0.7).unsqueeze(-1)
            wiou = self.wiou_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask], ret_iou=False, ratio=0.7, d=0.0, u=0.75, **{'scale':0.0}).unsqueeze(-1) # Wise-ShapeIoU,Wise-Inner-ShapeIoU,Wise-Focaler-ShapeIoU
            #wiou = self.wiou_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask], ret_iou=False, ratio=0.7, d=0.0, u=0.75, **{'mpdiou_hw':mpdiou_hw[fg_mask]}).unsqueeze(-1) # Wise-MPDIoU,Wise-Inner-MPDIoU,Wise-Focaler-MPDIoU
...
class v8DetectionLoss:
    ...
    def compute_loss(self, preds, batch):
        ...
        if isinstance(preds, dict):  # Detect/Segment/Pose/OBB heads return a dict during training
            feats = preds["feats"]
            pred_distri, pred_scores = preds["boxes"], preds["scores"]
        else:
            feats = preds[1] if isinstance(preds, tuple) else preds  # eval-mode tuple is (y, preds_dict)
            if isinstance(feats, dict):  # eval-mode preds dict {boxes, scores, feats, ...}
                pred_distri, pred_scores = feats["boxes"], feats["scores"]
                feats = feats["feats"]
            else:  # legacy list/tuple of per-level tensors
...
class E2ELoss:
    """Criterion class for computing training losses for end-to-end detection with decaying o2m weights."""
