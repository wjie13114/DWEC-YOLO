...
class WiseIouLoss(torch.nn.Module):                                                 ###我现在用的是行为识别 + 小数据集 + 类别不均衡，这会导致：
                                                                                    ###训练早期：大量低 IoU 框 梯度 → 不稳定；
                                                                                    ###训练后期：高 IoU 样本 贡献不足 → 精修不够
                                                                                    ##WiseIou让不同质量的样本，在不同阶段发挥“合适的作用”。

                                                            ###WiseIou 的思想就是不断维护一个 IoU 运行均值，比平均水平差的框 → 给更大梯度（重点优化），已经很好的框 → 不再过度拉扯。

                                                            ###ShapeIoU 负责提供更合理的几何约束，使预测框在位置和形状上更加贴近真实目标；
    ...
    ###传统 IoU 类损失只关心一件事：预测框和标准框重叠了多少;但是不关心 框的形状是否相似，框的长宽比例是否合理，行为目标（两只猪交互）是否被完整包住
    ###对猪行为识别来说，这是致命的，尤其是：aggressive（两只猪），lie / sit（姿态导致形状差异很大）
    ###Shapeiou = IoU + 位置约束 + 形状约束
    ...
    def _ShapeIoU(self, scale=0.0):
        ...
        ww = 2 * torch.pow(w2, scale) / (torch.pow(w2, scale) + torch.pow(h2, scale))               ###如果是“横向的猪”（w 大 h 小），那么 x 方向偏移更敏感；如果是“竖向的猪”，那么 y 方向偏移更敏感
        hh = 2 * torch.pow(h2, scale) / (torch.pow(w2, scale) + torch.pow(h2, scale))               ###这对我的数据极其重要，因为：lie / sit / aggressive 的形状方向性非常强
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        ...
        omiga_w = hh * torch.abs(w1 - w2) / torch.max(w1, w2)                                               ###小误差不敏感，大误差惩罚很重（防止畸形框）
        omiga_h = ww * torch.abs(h1 - h2) / torch.max(h1, h2)
        shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
